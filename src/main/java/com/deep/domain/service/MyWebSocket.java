...
public class MyWebSocket {
  ...
  public void onClose(@PathParam("user_id") Long user_id) {
    ...
    subOnlineCount();
    logger.info("Account " + user_id + " close!" + getOnlineCount() + " online");
  }
  ...
  public void onMessage(String message, Session session) throws IOException {
    ...
    sendMessage(msgBean.getMessage(), Long.valueOf(msgBean.getRole_id()), Long.valueOf(msgBean.getUser_id()), msgBean.getName(), msgBean.getTo(), msgBean.getMode(), msgBean.getTalk_id());
  }
   ...
   * @param talk_id 会话id
   */
  private void sendMessage(String message, Long role_id, Long user_id, String name, String[] tos, String mode, String talk_id) {
    switch (mode) {
      case "0": {
        //获取聊天对象的会话session
        ...
        Long target_id = initPrivateState(user_id, Long.valueOf(talk_id), role_id);
        //若对方不在线,记录聊天内容标记为未处理状态
        if (item == null) {
          this.getSession().getAsyncRemote().sendText(JSON.toJSONString(new ResponseBean(message, null, talk_id, name)));
          record(personalMap.get(target_id), name, user_id, Long.valueOf(talk_id), message, date, false, false);
        } else {
          if (isExpert(role_id) && accountMap.containsKey(Long.valueOf(talk_id)))
            this.getSession().getAsyncRemote().sendText(JSON.toJSONString(new ResponseBean("用户正在群聊中", null, talk_id, name)));
          else {
  ...
  public static void record(String talk_id, String talker_name, Long talker_id, Long receiver_id, String message, Date time, boolean isLink, boolean isHandle) {
    if (talkService.talkRecode(talk_id, talker_name, talker_id, receiver_id, message, time, isLink, isHandle) > 0)
      logger.info("insert into talk(talk_id, talker_name, talker_id, receiver_id, content, start_time, isHandle) values(\"" + talk_id + "\",\"" + talker_name + "\"," + talker_id + "," + receiver_id + ",\"" + message + "\",\"" + time + "\"," + isLink + "," + isHandle + ")");
    else
  ...
  public static Long initPrivateState(Long user_id, Long talk_id, Long role_id) {
    Long target_id = isExpert(role_id) ? talk_id : user_id;
